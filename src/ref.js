...
migolib.Goban.prototype.putstone = function(type, xidx, yidx) {
  ...
  //console.log(getstnumR, getstnumD, getstnumL, getstnumU)
  //console.log(getstnum)
  ...
  // 黒白の各個数
  let bwe_map = Array.prototype.concat.apply([], this.eyeArr_).map(x => x["st"]);
  ...
  for (var ix = 0; ix < this.eyeArr_.length; ix++) {
    for (var iy = 0; iy < this.eyeArr_[ix].length; iy++) {
      ...
      if (eye.st == migolib.con.eyest.b) {
        ...
        ctx.stoneimage = this.stonB
        //console.log(ctx.stoneimage.height)
      } else if (eye.st == migolib.con.eyest.w) {
        //ctx.fillStyle  = migolib.con.col.w;
        ctx.stoneimage = this.stonW
      } else {
        continue;
      }
      ctx.beginPath()
      //ctx.arc(eye.x, eye.y, this.stonR_, 0, migolib.con.PI2, true);
      ctx.drawImage(ctx.stoneimage, eye.x - this.stonR_, eye.y - this.stonR_, this.stonR_*2, this.stonR_*2);
      //ctx.fill();
      ctx.stroke();
    }
  }
